...
import com.guatai.yuntukubackend.model.enums.PictureReviewStatusEnum;
...
import com.guatai.yuntukubackend.utils.ColorSimilarUtils;
...
* @createDate 2026-03-09 20:53:57
*/
/**
 * @author 李鱼皮
 * @description 针对表【picture(图片)】的数据库操作Service实现
...
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture>
    ...
    public PictureVO uploadPicture(Object inputSource, PictureUploadRequest pictureUploadRequest, User loginUser) {
        ...
        ThrowUtils.throwIf(inputSource == null, ErrorCode.PARAMS_ERROR,"参数错误");
        // 判断是新增还是更新
        Long pictureId = null;
        if (pictureUploadRequest != null) {
            pictureId = pictureUploadRequest.getId();
        }
        // 如果是更新，判断图片是否存在，查询数据库中所有图片id与上传的id进行比对
        //如果找到相同的则放行，否则抛异常
        // 如果是更新图片，需要校验图片是否存在
        if (pictureId != null) {
        ...
        // 上传图片，得到图片信息
        // 按照用户 id 划分目录 目录结构为 存储桶/public/用户id
        // 按照用户 id 划分目录 => 按照空间划分目录
        String uploadPathPrefix;
        if (spaceId == null) {//没传spaceid默认公共
            uploadPathPrefix = String.format("public/%s", loginUser.getId());
        } else {//传了spaceId则按id划分上传目录
        ...
        picture.setUrl(uploadPictureResult.getUrl());//压缩图url
        picture.setThumbnailUrl(uploadPictureResult.getThumbnailUrl());//缩略图url
    ...
    public Page<PictureVO> getPictureVOPage(Page<Picture> picturePage, HttpServletRequest request) {
        ...
        // 初始化PictureVO分页对象，复用原分页参数（当前页、每页条数、总记录数）
        Page<PictureVO> pictureVOPage = new Page<>(picturePage.getCurrent(), picturePage.getSize(), picturePage.getTotal());
        // 空列表快速返回（避免后续无效操作）
        if (CollUtil.isEmpty(pictureList)) {
            return pictureVOPage;
        }
        // 对象列表 => 封装对象列表  list<picture> => List<pictureVO>
        List<PictureVO> pictureVOList = pictureList.stream()
                .map(PictureVO::objToVo)
                .collect(Collectors.toList());
        //此时的 PictureVO 仅包含图片自身字段，用户信息尚未填充。
        // 1. 关联查询用户信息
        //提取所有 userId→批量查询→分组 较传统遍历查询效率提高
        // 1,2,3,4 用set去重 取出用户id
        Set<Long> userIdSet = pictureList.stream().map(Picture::getUserId).collect(Collectors.toSet());
        // 1 => user1, 2 => user2 通过用户id分组对应用户信息
    ...
    public QueryWrapper<Picture> getQueryWrapper(PictureQueryRequest pictureQueryRequest) {
        ...
        queryWrapper.isNull(nullSpaceId, "spaceId");//spaceId为空则为查询公共图片
        if(endEditTime != null){
        ...
        // 排序 是否应用排序条件 排序方向（true表示升） 用于排序的字段（自定义）
    ...
    public void editPicture(PictureEditRequest pictureEditRequest, User loginUser) {
        ...
        // 注意将 list 转为 string 请求体和数据库字段不一样
    ...
    /***
     * 删除COS对象存储的方法 可以放在删除图片 跟新图片方法中
     * @param oldPicture
     */
    @Async//此注解可以使得删除可以被异步执行 但要配合主启动类的注解使用
    @Override
