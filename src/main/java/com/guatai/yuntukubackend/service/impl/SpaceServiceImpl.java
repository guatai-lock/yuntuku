...
public class SpaceServiceImpl extends ServiceImpl<SpaceMapper, Space>
    ...
    //引入spring编程式事务，相对于使用注解式事务，编程式事务锁粒度更细，性能更好，不用锁住整个方法
    @Resource
...
//    private DynamicShardingManager dynamicShardingManager;
    ...
    public long addSpace(SpaceAddRequest spaceAddRequest, User loginUser) {
        ...
        // 数据校验
        this.validSpace(space, true);//true代表创建时校验
        ...
        // 针对用户进行加锁，每个用户得到的锁不一样，提升系统性能
        //将用户 ID 转为字符串并放入字符串常量池，确保同一个用户的锁对象是唯一的
        String lock = String.valueOf(userId).intern();
        //基于用户 ID 的本地锁，同一用户的并发请求会排队执行，不同用户互不影响
        synchronized (lock) {
            //事务内执行数据库操作（要么全部成功，要么全部回滚）
            Long newSpaceId = transactionTemplate.execute(status -> {
                ...
                if(SpaceTypeEnum.TEAM.getValue() == spaceAddRequest.getSpaceType()){
                    ...
                    spaceUser.setSpaceRole(SpaceRoleEnum.ADMIN.getValue());//创建人直接为管理员
...
//                dynamicShardingManager.createSpacePictureTable(space);
    ...
    public void validSpace(Space space, boolean add) {
        ...
        if(spaceType != null && spaceTypeEnum == null){
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "创建的空间类型不存在");
        }
    }
    @Override
    public void fillSpaceBySpaceLevel(Space space) {
        // 根据空间级别，自动填充限额（空间本身没有设置限额时），有人工设置则已人工为主
