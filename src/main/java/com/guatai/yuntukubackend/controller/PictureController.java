...
import com.guatai.yuntukubackend.manger.auth.annotation.SaSpaceCheckPermission;
...
import com.guatai.yuntukubackend.model.vo.PictureTagCategory;
...
public class PictureController {
    ...
    public BaseResponse<Boolean> updatePicture(@RequestBody PictureUpdateRequest pictureUpdateRequest,
        ...
        //补充审核参数 修改图片后图片变为待审核状态，管理员修改会直接通过
        User loginUser = userService.getLoginUser(request);
    ...
    public BaseResponse<Page<PictureVO>> listPictureVOByPage(@RequestBody PictureQueryRequest pictureQueryRequest,
        ...
        //公共图库
        if(spaceId == null){
            //普通用户默认只能查看已过审的公开数据 默认设置只查询pass的图片
            pictureQueryRequest.setReviewStatus(PictureReviewStatusEnum.PASS.getValue());
            pictureQueryRequest.setNullSpaceId(true);//查公共图片
        }else{
            boolean hasPermission = StpKit.SPACE.hasPermission(SpaceUserPermissionConstant.PICTURE_VIEW);
            ThrowUtils.throwIf(!hasPermission, ErrorCode.NO_AUTH_ERROR);
        }
    ...
    /**
     * 使用多级缓存策略获取picture（VO）第一级本地缓存 第二级分布式缓存 最后查数据库
    ...
    public BaseResponse<Page<PictureVO>> listPictureVOByPageWithCache(@RequestBody PictureQueryRequest pictureQueryRequest,
        ...
        // 普通用户默认只能查看已过审的数据
        pictureQueryRequest.setReviewStatus(PictureReviewStatusEnum.PASS.getValue());
        // 构建缓存 key 项目名称：接口名称：查询条件（md5算法将原条件进行改造）
